...
//         `There are ${productSnap.size} products in this category. Please reasign those products to other categorioes before deleting.`
//       );
//     }
...
//   try {
//     if (!slug) {
//       return toast.error("Slug not found.");
//     }
...
export const getProductByCategoryAction = (parentCat) => async (dispatch) => {
  try {
    ...
    if (productSnap?.size > 0) {
      return toast.warning(
        `There are ${productSnap.size} products in this category. Please reassign those products to other categories before deleting.`
      );
    }
...
export const getSelectedProductsAction = (slug) => async (dispatch) => {
  try {
    ...
    const selectedProdRef = doc(db, "products", slug);
    const selectedProdSnap = await getDoc(selectedProdRef);
...
export const addProductAction =
  ...
  async (dispatch) => {
    try {
      const promise = setDoc(doc(db, "products", slug), rest, { merge: true });
